...
from src.schemas import TourSchema  # Создадим схему для туров
...
class Tour(Base):
    ...
    @classmethod
    async def add_tour(cls, name: str, description: str):
        """
        Добавляет новый тур в базу данных.
        """
        async with async_session_maker() as session:
            existing_tour = await session.execute(select(cls).where(cls.name == name))
            if existing_tour.scalars().first():
                return False 
    ...
    @classmethod
    async def delete_tour(cls, tour_id: int):
        """
    ...
    @classmethod
    async def get_tour(cls, tour_id: int) -> TourSchema:
        """
        Возвращает информацию о туре по его ID.
        """
        async with async_session_maker() as session:
            tour = await session.get(cls, tour_id)
            return TourSchema(
    ...
    @classmethod
    async def get_all_tours(cls) -> list[TourSchema]:
        """
